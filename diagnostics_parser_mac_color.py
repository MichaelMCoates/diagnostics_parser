...
class ParseDebugLog(object):
    ...
    def print_to_console(self):
        ...
        for idx, application in enumerate(self.applications):
            print("\n")
            prGreen(f"Application #{idx + 1}")
            pprint.pprint(application)

        print("\n--------------------XXX--------------------")

        prYellow("\nEntities:")
        prYellow("Entities sorted by time_started")
        ...
        for application_dict_key in self.entities:
            print("\n")
            print("\n")
            prYellow(f"Application UUID: {application_dict_key}")
            ...
            for idx, value in enumerate(sorted(application_dict.values(), key=by_time_started)):
                print("\n")
                prYellow(f"Entity #{idx + 1} for {application_dict_key}: ")
                pprint.pprint(value)

        print("\n--------------------XXX--------------------")

        prLightPurple("\nEntities by Render Frame ID:")
        prLightPurple("Entities sorted by time_started")

        for render_frame_id in self.render_frames:
            print("\n")
            print("\n")
            prLightPurple(f"Render Frame ID: {render_frame_id}")
            ...
            for idx, value in enumerate(sorted(render_frame_dict.values(), key=by_time_started)):
                print("\n")
                prLightPurple(f"Entity #{idx + 1} for {render_frame_id}: ")
                pprint.pprint(value)
        ...
        for idx, console_log in enumerate(self.console_logs):
            print("\n")
            prPurple(f"Console Log #{idx + 1}")
            prPurple(console_log)
